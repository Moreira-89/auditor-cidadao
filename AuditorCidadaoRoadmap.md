...
**🆕 Decisão (2026-07-14): trocar direto para `RedisSaver` (`langgraph-checkpoint-redis`), não manter `InMemorySaver` com eviction/TTL manual.** O plano anterior (enxertar TTL no `InMemorySaver`, no mesmo padrão do `cachetools.TTLCache` já aplicado ao cache de tools no Bloco 0) foi discutido e descartado: resolveria só a expiração, mas não o problema de fundo — estado de conversa preso à RAM de um único processo, que é exatamente o que bloqueia o item "Workers e escalonamento horizontal" acima. `RedisSaver` resolve TTL nativo **e** consistência entre workers/instâncias numa peça só, então é a solução mais direta para o gap de escalabilidade, não só um adiamento dele.

**Ordem de execução — deliberadamente separada da Seção B:** trocar o checkpointer para `RedisSaver` agora, isolado, validando com o golden dataset do Bloco 4 antes de tocar em qualquer outra coisa — **sem** esperar a migração para `create_agent` (Seção B) para fazer as duas mudanças juntas. A Seção B originalmente sugeria as duas mudanças juntas (ver nota da linha do checkpointer nessa seção); a decisão agora é separar. Motivo: são dois ciclos de validação em vez de um, mas isola o risco de cada mudança, e a troca do checkpointer resolve um gap de escalabilidade que a banca já tende a questionar (T6) — a migração para `create_agent` é dívida de idiomaticidade, não um gap identificado em apresentação. Sem pressão de prazo (ver nota no topo do documento), não há razão para segurar a correção mais urgente esperando a mais "arquiteturalmente elegante". Se ambas forem feitas no mesmo ciclo por conveniência, tudo bem — só não é mais a ordem sugerida por este roadmap.

| Componente | V1 (atual) | V2 (alvo) |
|---|---|---|
| Histórico de conversas | `InMemorySaver` (RAM), sem TTL/eviction | `RedisSaver` (`langgraph-checkpoint-redis`) — TTL nativo por thread, resolvendo o crescimento indefinido do dict em RAM **e**, ao mesmo tempo, o pré-requisito de estado compartilhado entre múltiplos workers/instâncias (ver item de Workers acima) |
| Cache de tools | Dict/`TTLCache` em memória | Redis com TTL nativo, compartilhado entre instâncias — pode reaproveitar a mesma instância Redis do checkpointer acima; só relevante se a V2 realmente escalar para múltiplos workers/instâncias (ver item acima) |
...
> **Importante não confundir os dois TTLs acima:** hoje (V1), o `InMemorySaver` guarda o *estado da conversa* (RAM do processo, some sozinho se o servidor reiniciar); o Pinecone guarda os *vetores do edital indexado* (serviço externo, **não** é afetado por restart do Railway). São dois ciclos de vida independentes — reiniciar a aplicação não limpa o Pinecone, e o Pinecone não sabe quando uma sessão de chat "acabou" sem um mecanismo de expiração próprio. Depois da migração para `RedisSaver` acima, o estado da conversa também passa a viver num serviço externo (não some mais com o restart do processo) — mas continua sendo um ciclo de vida distinto do Pinecone, cada um com seu próprio TTL.
...
- **Checkpointer e singleton.** 🆕 **Atualização (2026-07-14):** a troca de checkpointer (`InMemorySaver` → `RedisSaver`, ver Seção A) passou a ser conduzida **isolada e antes** desta migração, não junto com ela — decisão registrada na seção de Persistência, revertendo a sugestão original deste item. O que permanece válido aqui é só o padrão singleton (`initialize_graph`/`get_graph` em `build_graph.py`) vs. formas recomendadas de gerenciar ciclo de vida do grafo numa aplicação FastAPI — reavaliar esse ponto depois que o `RedisSaver` já estiver em produção, não antes.
...
|---|---|---|---|
| 1 | `InMemorySaver` perde histórico a cada restart | Baixo (MVP) | Backlog V2: `RedisSaver` (`langgraph-checkpoint-redis`) — decisão atualizada em 2026-07-14, ver Seção A → Persistência (era `PostgresSaver` numa versão anterior deste roadmap) |
...
| 14 | Sem autenticação nem limite de uso: usuário pode conversar (gastar tokens da OpenAI) indefinidamente | Alto | Confirmado: nenhum middleware, `Depends` de auth ou rate limiter em `app/`. Backlog V2: rate limiting/quota/timeout de conversa — ver seção "Controle de custo e limite de uso" |