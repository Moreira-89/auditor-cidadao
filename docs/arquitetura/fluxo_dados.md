...
um edital (upload) e a conversa com o agente (pergunta → laudo). Separados em dois diagramas — no
[Visão Geral](visao_geral.md) eles estavam combinados num único fluxograma e ficaram pequenos
demais para ler.
...
flowchart TB
    ...
    AGENTE --> RF["consultar_receita_federal"]
...
forem necessárias, antes de responder — o funcionamento interno desse loop (`call_llm` ↔
`tool_node` ↔ `router`) está detalhado em [Visão Geral](visao_geral.md#o-ciclo-de-decisao-do-agente).
